    ...
    for (const pattern of patterns) {
        ...
        while (match) {
            ...
            if (Number.isFinite(step) && Number.isFinite(totalSteps) && totalSteps > 0) {
                events.push({ step, totalSteps, progress: Math.min(1, step / totalSteps) });
            }
...
function parseGenerationProgressChunk(chunk, state = { tail: '', lastStep: 0, lastTotalSteps: 0 }) {
    ...
    for (const event of events) {
        if (event.totalSteps !== state.lastTotalSteps) { state.lastTotalSteps = event.totalSteps; state.lastStep = 0; }
        if (event.step > state.lastStep) { state.lastStep = event.step; freshEvents.push(event); }
    }
    state.tail = combined.slice(-1024);
    return freshEvents;
}

module.exports = { coerceFiniteNumber, extractProgressEvents, formatStartupProgressMessage, parseGenerationProgressChunk, resolveGenerationSteps, resolveGuidanceScale, stripAnsiSequences };

...
function pickBinaryAssetForPlatform({ platform, arch, zipNames }) {
    ...
    if (platform === 'win32') {
        ...
        for (const tag of order) { const hit = winCandidates.find((name) => name.includes(tag)); if (hit) return hit; }
        return null;
    }
    if (platform === 'linux' && arch === 'arm64') {
        const linuxArmCandidates = candidates.filter((name) => name.includes('Linux') && (name.includes('aarch64') || name.includes('arm64')));
        const plain = linuxArmCandidates.find((name) => !name.includes('rocm') && !name.includes('vulkan'));
        return plain || linuxArmCandidates.find((name) => name.includes('vulkan')) || linuxArmCandidates.find((name) => name.includes('rocm')) || null;
    }
    ...
    return plain || linuxCandidates.find((name) => name.includes('vulkan')) || linuxCandidates.find((name) => name.includes('rocm')) || null;
}
...
module.exports = { getBundledBinaryResourceDir, pickBinaryAssetForPlatform };

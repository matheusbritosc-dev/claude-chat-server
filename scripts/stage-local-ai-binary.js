...
function resolveSourceBinDir(sourcePath) {
    ...
    if (fs.existsSync(nestedBinDir) && fs.statSync(nestedBinDir).isDirectory()) return nestedBinDir;
...
function stageLocalAiBinary({ platform, arch, sourcePath }) {
    ...
    if (!requiredFiles) throw new Error(`Unsupported platform "${platform}". Expected one of: ${Object.keys(REQUIRED_FILES).join(', ')}`);
    const missingFiles = requiredFiles.filter((fileName) => !fs.existsSync(path.join(sourceBinDir, fileName)));
    if (missingFiles.length > 0) throw new Error(`Missing required files in ${sourceBinDir}: ${missingFiles.join(', ')}`);
    ...
    for (const fileName of [...requiredFiles, ...OPTIONAL_FILES]) {
        ...
        if (platform !== 'win32') fs.chmodSync(targetFile, 0o755);
    }
    return stageDir;
}
...
function main() {
    ...
    if (!platform || !arch || !sourcePath) { console.error('Usage: node scripts/stage-local-ai-binary.js <platform> <arch> <source-path>'); process.exit(1); }
    try {
...
module.exports = { resolveSourceBinDir, stageLocalAiBinary };

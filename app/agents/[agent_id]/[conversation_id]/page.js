...
async function fetchHistory(agentId, conversationId, apiKey) {
  if (!apiKey) return null;
  try {
    const res = await fetch(`${BASE_URL}/agents/by-slug/${agentId}/${conversationId}`, { cache: "no-store", headers: { "x-api-key": apiKey } });
    if (res.ok) return await res.json();
    if (agentId.length > 20) {
      const resId = await fetch(`${BASE_URL}/agents/${agentId}/${conversationId}`, { cache: "no-store", headers: { "x-api-key": apiKey } });
      if (resId.ok) return await resId.json();
    }
    return null;
  } catch { return null; }
...
export default async function AgentConversationPage({ params }) {
  ...
  const [agentDetails, initialHistory, userData] = await Promise.all([fetchAgentDetails(agent_id, apiKey), fetchHistory(agent_id, conversation_id, apiKey), fetchUserData(apiKey)]);
